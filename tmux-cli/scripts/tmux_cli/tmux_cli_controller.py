...
def _load_help_text():
    ...
    try:
        ...
        # Try looking in the parent directory (plugin root) for docs
        ...
        if hasattr(importlib.resources, 'files'):
            ...
            # Try docs as a subdirectory within the package
            try:
                help_file = resources.files('tmux_cli') / 'docs' / 'tmux-cli-instructions.md'
...
Use `tmux-cli --help` for detailed documentation.

## Basic Commands
- tmux-cli launch "command" - Launch CLI in new pane
- tmux-cli send "text" --pane=ID - Send input to pane
- tmux-cli capture --pane=ID - Capture output
- tmux-cli execute "cmd" --pane=ID - Run with exit code
- tmux-cli status - Show status
"""
...
class CLI:
    ...
    def send(self, text: str, pane: Optional[str] = None, enter: bool = True,
        ...
        Args:
            ...
            delay_enter: If True, use 1.5s delay; if float, use that delay in seconds (default: True)
            verify_enter: If True, verify Enter was received and retry if not (default: True).
                          Set to False for interactive programs that may take time to render.
        """
        if self.mode == 'local':
            ...
            self.controller.send_keys(text, pane_id=pane, enter=enter,
                                    delay_enter=delay_enter, verify_enter=verify_enter)
        print("Text sent")
