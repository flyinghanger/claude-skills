...
`~/.claude/skills/` is the single source of truth for all skills. Other agents (Codex, Cursor, etc.) use symlinks pointing to it. Always ensure skills are installed to `~/.claude/skills/`.
...
### Post-install: Symlink to other agents

After installation, create symlinks for other agents that share skills:
...
- Offer to symlink to other agents (e.g., Codex) if applicable
